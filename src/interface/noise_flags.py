...
# Parser for flags
    ...
    parser.add_argument('--units', choices=['lj','real','metal','si','cgs','electron','micro','nano'], default='lj',  \
                        help='pass the units used in the simulation for all the variables. Default value is \' lj \' ')
    
    parser.add_argument('--step_length', action='store', default=0.005, type=float, \
                        help='pass the timesteps\' length. It is a float variable, with default value of 0.005 .')
                        
    parser.add_argument('--step_number', action='store', default=5000, type=int, \
                    help='pass the number of timesteps for integration, int with a default value of 5000.')
    ...
    parser.add_argument('--lj_coeff', action= 'append',type=float,nargs='+',default=[2.5,0.8,1.2],  help='Pass the needing coefficients for defining the lj potential; the parameters needed are the cutoff, epsilon, sigma. The default values are: 2.5,0.8,1.2 .')
    ...
    parser.add_argument('--box', action= 'append',type=float,nargs='+',default=[-60, 60 , -60, 60,-60,60],  help='Pass the needing for defining the simulation box; the parameters needed are the xlo,xhi,ylo,yhi,zlo,zhi. The default values are: -60, 60, -60, 60, -60, 60.')
